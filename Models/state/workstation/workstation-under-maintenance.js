...
class UnderMaintenanceWorkstation extends Workstation {
    constructor(id, partsList, tpm, workstations, maintenanceDuration = 3000) { // Default duration in milliseconds (3 seconds)
      super(id, partsList);
    ...
    addPartToCar() {
      const remainingTime = Math.max(0, Math.ceil((this.maintenanceDuration - (Date.now() - this.startTime)) / 1000));
      throw new Error(`Cannot add part, workstation ${this.id} is under Maintenance for ${remainingTime} seconds remaining.`);
    }
